...
def get_document_content(browser, abstract, document):
    ...
    document_table_body = locate_element_by_tag_name(document_information, abstract.county.tags["Table Body"],
...
def record_book_and_page(abstract, rows):
    ...
    if book_and_page == 'N/A':
        ...
        if book == '0' and page == '0':
            ...
            record_value(abstract, 'book', book.strip())
            record_value(abstract, 'page', page.strip())
...
def record(browser, abstract, document):
    if not abstract.review:
        ...
        if abstract.download_only:
            print('Abstractor has not developed a "download_only" path to date, please review and try again.')
