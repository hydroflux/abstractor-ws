...
# Need further review to determine if "Book" and "Page" fields even exist for jaguar
def split_reception_field(reception_number_field_map):
    reception_number_field_list = list(reception_number_field_map)
    if len(reception_number_field_list) == 3:
        reception_number_field, _, _ = reception_number_field_list
    else:
        reception_number_field, _ = reception_number_field_list
    fields = reception_number_field.split('\n')
    reception_number = fields[0]
    recording_date = fields[2][:10]
    return reception_number, recording_date
...
def access_download_information(browser, abstract, document):
    ...
    document_tables = access_document_tables(browser, abstract, document)
    reception_number_field_map = access_indexing_information(abstract, document_tables[1])
    reception_number, _ = split_reception_field(reception_number_field_map)
    document.reception_number = reception_number
    document.download_value = f'{document.reception_number}-{abstract.county.other["Stock Download"]}'
...
def record(browser, abstract, document):
    if not abstract.review:
        if abstract.download_only:
            ...
            input("Abstractor currently does not have a logical path to perform recording functions, please review...")
        # else:
        #     record_comments(abstract, document)  # Before 'handle_document_image_status' to check multiple documents
        #     handle_document_image_status(browser, abstract, document)
        #     record_document_fields(browser, abstract, document)
        #     record_empty_values(abstract, ['effective date', 'volume', 'document link'])
        ...
        input("Abstractor currently does not have a logical path to perform review functions, please review...")
