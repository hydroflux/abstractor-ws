...
def record_reception_number(abstract, row, document):
    reception_number = get_row_value(row, abstract.county.titles["Row Titles"]["reception_number"])
...
def record_book_and_page(abstract, row):
    book_page_value = get_row_value(row, abstract.county.titles["Row Titles"]["book_and_page"])
...
def record_recording_date(abstract, row):
    recording_date = get_row_value(row, abstract.county.titles["Row Titles"]["recording_date"])
    record_value(abstract, 'recording date', recording_date[:10])


def record_document_type(abstract, row):
    document_type = get_row_value(row, abstract.county.titles["Row Titles"]["document_type"])
    record_value(abstract, 'document_type', document_type.title())


def record_grantor(abstract, row):
    grantor = get_row_value(row, abstract.county.titles["Row Titles"]["grantor"])
    record_value(abstract, 'grantor', grantor.title())


def record_grantee(abstract, row):
    grantee = get_row_value(row, abstract.county.titles["Row Titles"]["grantee"])
    record_value(abstract, 'grantee', grantee.title())


def record_related_documents(abstract, row):
    related_documents = get_row_value(row, abstract.county.titles["Row Titles"]["related_documents"])
    record_value(abstract, 'related documents', related_documents)


def record_legal(abstract, row_1):
    legal = get_row_value(row_1, abstract.county.titles["Row Titles"]["legal"])
